...
describe("Dashboard Controller", () => {
  ...
  it("deve negar acesso ao dashboard do admin para usuário comum", async () => {
    // Garante email único
    const user = await TestDataFactory.createUser({
      email: "user_dashboard_" + Date.now() + "@test.com",
      username: "user_dashboard_" + Date.now(),
    });
    const userToken = TestDataFactory.generateAuthTokens(user).accessToken;
    await request(app)
      .get("/dashboard/admin")
  ...
  it("deve negar acesso sem token", async () => {
    await request(app).get("/dashboard/client").expect(401);
    await request(app).get("/dashboard/admin").expect(401);
  });
