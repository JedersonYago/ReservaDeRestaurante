...
describe('dateUtils', () => {
  ...
  it('isWithinTableAvailability deve validar datas e horários', () => {
    ...
    expect(isWithinTableAvailability(today.toISOString(), '12:00', from.toISOString(), to.toISOString())).toBe(true);
    expect(isWithinTableAvailability(today.toISOString(), '10:00', from.toISOString(), to.toISOString())).toBe(false);
  });
