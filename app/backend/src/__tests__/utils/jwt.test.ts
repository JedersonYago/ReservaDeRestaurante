import { generateTokenPair, verifyToken, verifyRefreshToken, isTokenExpired } from '../../utils/jwt';
...
beforeAll(() => {
  jest.spyOn(TestDataFactory, 'createUser').mockImplementation(async (data = {}) => ({ ...fakeUser, ...data }) as unknown as import('../../models/User').IUser);
...
describe('JWT Utils', () => {
  ...
  describe('verifyRefreshToken', () => {
    ...
    it('deve lançar erro para access token usado como refresh token', () => {
      ...
      expect(() => verifyRefreshToken(accessToken)).toThrow('Token não é um refresh token');
    });
  ...
  describe('isTokenExpired', () => {
    ...
    it('deve retornar true para token sem a claim exp', () => {
      const tokenWithoutExp = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIn0.invalid';
