import { applyConfigToReservation, generateAvailableTimeSlots } from '../../utils/configUtils';
...
describe('configUtils', () => {
  ...
  it('applyConfigToReservation deve aceitar reserva dentro do horário', async () => {
    const result = await applyConfigToReservation(baseConfig, '2025-07-25', '12:00', []);
    expect(result.isValid).toBe(true);
  });

  it('applyConfigToReservation deve rejeitar reserva antes do horário de abertura', async () => {
    const result = await applyConfigToReservation(baseConfig, '2025-07-25', '10:00', []);
  ...
  it('applyConfigToReservation deve rejeitar reserva após o horário de fechamento', async () => {
    const result = await applyConfigToReservation(baseConfig, '2025-07-25', '23:30', []);
    expect(result.isValid).toBe(false);
